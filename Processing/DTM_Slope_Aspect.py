...
def create_point_cloud(input_file):
    ...
    cmd = "saga_cmd -f=s io_shapes 16 -POINTS={1} -FILE={0} -FIELDS='4;5' -FIELDNAMES='intensity;classification' -FIELDTYPES="2;2" -FIELDSEP=2".format(input_file, output_file)
    sb.call(cmd, shell=False)
    return output_file
...
def ground_points(input_file):
    ...
    cmd = "saga_cmd -f=s pointcloud_tools 6 -INPUT={0} -ATTRIB='classification' -RESULT={1} -MODE=1 -OLD=2 -SOPERATOR=2".format(input_file, output_file)
    sb.call(cmd, shell=False)
    return output_file
...
def create_grid(input_file):
    ...
    cmd = "saga_cmd -f=s pointcloud_tools 4 -POINTS={0} -OUTPUT=0 -GRID={1} -AGGREGATION=3 -CELLSIZE=1.000000".format(input_file, output_file)
    sb.call(cmd, shell=False)
...
def DTM_filter(input_file):
    ...
    cmd = "saga_cmd -f=s grid_filter 7 -INPUT={0} -RADIUS=10 -TERRAINSLOPE=75 -STDDEV -GROUND={1}".format(input_file, output_file)
    sb.call(cmd, shell=False)
    return output_file
...
def fill_grid_gaps(input_file):
    ...
    cmd = "saga_cmd -f=s grid_tools 7 -INPUT={0} -RESULT={1} -THRESHOLD=0.100000".format(input_file, output_file)
    sb.call(cmd, shell=False)
    return output_file
...
def slope_aspect(input_file):
    ...
    cmd = "saga_cmd -f=s ta_morphometry 0 -ELEVATION={0} -SLOPE={1} -ASPECT={2}".format(input_file, output_file1, output_file2)
    sb.call(cmd, shell=False)
    return output_file
